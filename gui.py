...
#         self.db = db
#         self.root = tk.Tk()
#         self.root.title("EquityVision - Visualizzazione Raccomandazioni")
...
#         # Treeview
#         columns = ('Azienda', 'Banca', 'Target Price', 'Data')
...
class EquityVisionGUI:
    ...
    # --- Funzione Aggiunta ---
    def show_chart(self):
        """Mostra un grafico dei target price nel tempo"""
        if not self.filtered_data:
            messagebox.showwarning("Attenzione", "Nessun dato da visualizzare!")
            return
        
        # Verifica che sia selezionata una singola azienda
        selected_company = self.name_var.get()
        if not selected_company or selected_company == 'Tutte':
            messagebox.showwarning("Attenzione", "Seleziona una singola azienda per visualizzare il grafico!")
            return
        ...
        try:
            ...
            for row in self.filtered_data:
                ...
                # Aggiungi alla lista dei prezzi per quella data
                date_prices[date_obj].append(target_price)
            
            if not date_prices:
                messagebox.showwarning("Attenzione", "Nessun dato valido da visualizzare!")
                return
            
            # Calcola la media per ogni data (come richiesto)
            dates = sorted(date_prices.keys())
            avg_prices = [sum(date_prices[d]) / len(date_prices[d]) for d in dates]
            
            # Crea il grafico
            plt.figure(figsize=(12, 6))
            plt.plot(dates, avg_prices, marker='o', linestyle='-', linewidth=2, markersize=8, color='#2E86AB')
            
            # Configura il grafico (come richiesto)
            plt.title(f'Andamento Target Price - {selected_company}', fontsize=16, fontweight='bold', pad=20)
            plt.xlabel('Data', fontsize=12, fontweight='bold')
            plt.ylabel('Euro', fontsize=12, fontweight='bold') # Etichetta asse Y
            plt.legend(['Target Price'], loc='best', fontsize=10) # Legenda
